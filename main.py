...
def read(record_id):
    try:
        ...
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute(
            "SELECT id, name, description FROM records WHERE id = %s", record_id)
...
@app.route('/data/<int:record_id>', methods=['PATCH'])
def update(record_id):
    try:
        json = request.json
        name = json['name']
        description = json['description']
        if name and description and record_id and request.method == 'PATCH':
